        ...
        <div className="container">
          ...
          <div className={cx("text-center", project?.link ? "" : "d-none")}>
            <Link to={project?.link ? project?.link : ""} target="_blank">
              <button className="btn btn-lg btn-secondary fs-4 px-5 py-3 shadow-lg text-uppercase case-study">
                Project Link
              </button>
            </Link>
          </div>
      ...
      <div className="section-2">
        <div className="container pt-5 mt-3">
          ...
          <div className="row mt-4">
            <div className="col-md-8 m-auto">
              <div className="fs-3 fw-bolder mb-2">Project Overview</div>
              <div
                className={cx(
                  "text-justify",
              ...
              >
                {project?.description.map(
                  (description: string, index: number) => (
                    <Description key={index} htmlString={description} />
                  ),
                )}
              </div>
              <div className="fs-3 fw-bolder mt-5 mb-4">Tools Used</div>
              <div className="d-flex justify-content-start flex-wrap gap-3 my-4">
                {project?.tools?.map((tool: string, index: number) => {
                  return (
                    <button
                      className="border-0 btn btn-light btn-outline-secondary"
                      key={index}
                    >
                      {tool}
                    </button>
